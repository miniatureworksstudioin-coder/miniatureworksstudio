...
export const cleanOrder = (body) => ({
  ...
  delivery: body.delivery && typeof body.delivery === "object" ? body.delivery : null,
  createdAt: new Date().toISOString(),
  status: body.status || (body.orderType === "custom" ? "Awaiting quote" : "Received"),
});

export const validateOrder = (order) => {
  if (!validOrderId(order.orderId)) return "Invalid order ID.";
  if (!order.name || order.phone.length < 10) return "Name and a valid phone number are required.";
  if (order.orderType === "catalog" && (!order.items.length || !order.address || !order.city || !/^\d{6}$/.test(order.pincode))) {
    return "A catalog order needs items, an address, a city and a 6-digit pincode.";
  }
  if (order.orderType === "custom" && !order.description) return "A custom order needs a description.";
  return null;
};